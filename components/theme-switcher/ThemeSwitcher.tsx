...
@template(function({compact}) {
	...
	return <shadow-root>
			{compact ? <button data-mode={mode} id="button" class="compact-wrapper">
				...
				<span>
					<input id={id + "-dark"} type="radio" value="dark"/>
					<label for={id + "-dark"}>
	...
	private initDefault() {
		...
		inputs.forEach((input) => {
			input.addEventListener("input", () => {
				const mode = input.value === "system" ? 
					(globalThis.matchMedia('(prefers-color-scheme: dark)').matches ? "dark" : "light") :
					input.value as "light" | "dark";
				inputs.forEach((other) => {
					if (other !== input)
						other.checked = false;
				});
				UIX.Theme.setMode(mode);
			});
		});
